  ...
  List<SalesRecord> _rowsToRecords(List<List<dynamic>> rows) {
    ...
    for (int i = 1; i < rows.length; i++) {
      ...
      try {
        ...
        final unitPrice =
            idxUnit >= 0 ? double.tryParse(r[idxUnit].toString()) ?? 0.0 : 0.0;
        final total = idxTotal >= 0
            ? double.tryParse(r[idxTotal].toString()) ?? (qty * unitPrice)
            : (qty * unitPrice);
